...
if (length(argv) == 1) {
    ...
    stop(call. = FALSE,
         paste0("Wrong number of arguments.\n\n",
...
p2 <- ggplot(barcodes_df, aes(Rank, Count)) +
    ...
    scale_x_continuous(labels=function(x) trimws(format(x, big.mark = ","))) +
...
outfile <- paste0(argv[1], ".pdf")
pdf(outfile, width = 4*1.61803, height = 4)
print(p1)
print(p2)
print(p3)
graphics.off()
message("PDF produced: ", outfile)
